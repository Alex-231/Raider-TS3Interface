...
void onClientConnected(uint64 serverID, anyID clientID, uint64 channelID, unsigned int* removeClientError) {
	SharedAPI::Log("Client joined channel on virtual server");
	SharedAPI::Log((char*)std::to_string(clientID).c_str());
	SharedAPI::Log((char*)std::to_string(channelID).c_str());
	SharedAPI::Log((char*)std::to_string(serverID).c_str());
}
...
void onClientDisconnected(uint64 serverID, anyID clientID, uint64 channelID) {
	SharedAPI::Log("Client left channel on virtual server");
	SharedAPI::Log((char*)std::to_string(clientID).c_str());
	SharedAPI::Log((char*)std::to_string(channelID).c_str());
	SharedAPI::Log((char*)std::to_string(serverID).c_str());
}
...
bool StartServer(char* ipAddr, int port, char* serverName, ServerLibFunctions callbacks) {
